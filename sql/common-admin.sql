...
CREATE TABLE `rc_base_area` (
  ...
  `parent_code` varchar(255) DEFAULT NULL COMMENT '父区域编码',
...
CREATE TABLE `rc_dict` (
  ...
  PRIMARY KEY (`id`)
) ENGINE=InnoDB AUTO_INCREMENT=39 DEFAULT CHARSET=utf8mb4 COMMENT '字典表';
...
CREATE TABLE `rc_menu` (
  ...
  `code` varchar(180) DEFAULT NULL COMMENT '菜单编码',
...
CREATE TABLE `rc_role` (
  `id` int(11) NOT NULL AUTO_INCREMENT COMMENT '主键',
  `name` varchar(180) DEFAULT NULL COMMENT '角色名',
  `value` varchar(180) DEFAULT NULL COMMENT '角色值',
...
drop table if exists `c_client_user` ;
create table `c_client_user` (
`client_id`  int(11) unsigned zerofill not null auto_increment comment '主键 ' ,
`client_name`  varchar(100) not null default '未知企业' comment '企业名称' ,
`client_mobile`  varchar(20) not null default '9999999' comment '企业联系电话' ,
`linkman`  varchar(20) not null default '默认联系人' comment '联系人' ,
`link_phone`  varchar(15) not null default '999999999' comment '联系人电话' ,
`login_name`  varchar(20) not null default 'testuser' comment '登录名称' ,
`login_passwd`  varchar(64) not null default '123456' comment '登录密码 ' ,
`status`  varchar(1) not null default '1' comment '状态 : 1 正常 2黑名单 3 停用' ,
...
drop table if exists `c_price_group` ;
create table `c_price_group` (
`price_id`  int(11) unsigned zerofill not null auto_increment comment '主键 ' ,
`price`  int(10) not null default 10 comment '分/连接数' ,
`status`  varchar(1) not null default '1' comment '状态 : 1 正常 2 停用' ,
`create_on`  timestamp  comment '创建时间' ,
`create_by`  varchar(20) null comment '创建人' ,
`update_no`  timestamp  comment '更新时间' ,
`update_by`  varchar(20) null comment '更新人' ,
primary key (`price_id`)
)  engine=innodb default charset=utf8mb4 comment '价格组表';

drop table if exists `c_gateway` ;
create table `c_gateway` (
`gateway_id`  int(11) unsigned zerofill not null auto_increment comment '主键 ' ,
`gateway_ip`  varchar(100) not null default '0.0.0.0' comment '网关ip' ,
`gateway_port` varchar(50) not null default '9999' comment '网关端口' ,
`status`  varchar(1) not null default '1' comment '状态 : 1 正常 2 停用' ,
`create_on`  timestamp  comment '创建时间' ,
`create_by`  varchar(20) null comment '创建人' ,
`update_no`  timestamp  comment '更新时间' ,
...
drop table if exists `c_client_gateway_price_relation` ;
create table `c_client_gateway_price_relation` (
`relation_id`  int(11) unsigned zerofill not null auto_increment comment '主键 ' ,
`client_id`  int(11) not null  comment '客户id' ,
`gateway_ip`  int(11) not null  comment '网关id' ,
`price_id`  int(11) not null  comment '价格组id' ,
`create_on`  timestamp  comment '创建时间' ,
`create_by`  varchar(20) null comment '创建人' ,
`update_no`  timestamp  comment '更新时间' ,
`update_by`  varchar(20) null comment '更新人' ,
primary key (`relation_id`)
)  engine=innodb default charset=utf8mb4 comment '客户网关价格关系表';


drop table if exists `c_client_card_relation` ;
create table `c_client_card_relation` (
`cc_relation_id`  int(11) unsigned zerofill not null auto_increment comment '主键 ' ,
`client_id`  int(11) not null  comment '客户id' ,
`gateway_ip`  int(11) not null  comment '网关id' ,
`card_info`  int(11) not null  comment '卡信息' ,
`create_on`  timestamp  comment '创建时间' ,
`create_by`  varchar(20) null comment '创建人' ,
`update_no`  timestamp  comment '更新时间' ,
`update_by`  varchar(20) null comment '更新人' ,
primary key (`cc_relation_id`)
)  engine=innodb default charset=utf8mb4 comment '客户与卡关系表';



drop table if exists `c_hour_ticket` ;
create table `c_hour_ticket` (
`ticket_id`  int(11) unsigned zerofill not null auto_increment comment '主键 ' ,
`connect_id` varchar(64) comment '连接id',
`account` varchar(64) comment '账户',
`online_time`  timestamp  comment '上线时间' ,
`offline_time`  timestamp  comment '下线时间' ,
`flow_size` int(10) comment '流量大小',
`connect_ip` varchar(64) comment '连接ip',
`server_ip` varchar(64) comment '服务ip',
primary key (`ticket_id`)
)  engine=innodb default charset=utf8mb4 comment '实时话单表';
...
drop table if exists `c_month_bill` ;
create table `c_month_bill` (
`bill_id`  int(11) unsigned zerofill not null auto_increment comment '主键 ' ,
`client_id`  int(11) not null comment '企业id' ,
`bill_month` VARCHAR(30) not null  comment '账单月份' ,
`receivable_money` int(11) not null comment '应收金额 , 单位分 ' ,
`real_income_money` int(11) not null comment '实收金额 , 单位分 ' ,
`month_connected` int(11) not null comment '连接数 ' ,
`create_on`  timestamp  comment '创建时间' ,
`create_by`  varchar(20) null comment '创建人' ,
`update_no`  timestamp  comment '更新时间' ,
`update_by`  varchar(20) null comment '更新人' ,
primary key (`bill_id`)
)  engine=innodb default charset=utf8mb4 comment '月账单表';



drop table if exists `c_receive_money` ;
create table `c_receive_money` (
`receive_id`  int(11) unsigned zerofill not null auto_increment comment '主键 ' ,
`client_id`  int(11) not null comment '企业id' ,
`counteract_bill_month` VARCHAR(30) not null  comment '抵消月份' ,
`receive_money` int(11) not null comment '收款金额' ,
